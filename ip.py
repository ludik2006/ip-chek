...
<body>
    <div class="container">
        ...
        <div class="info-block">
            <h2>🔋 Батарея</h2>
            <p><strong>Заряд:</strong> <span id="batteryLevel"></span></p>
    ...
    <script>
        ...
        document.getElementById('colorDepth').textContent = `${window.screen.colorDepth} бит`;

        // GPU (только в Chrome)
        if (navigator.gpu) {
            navigator.gpu.requestAdapter().then(adapter => {
                document.getElementById('gpu').textContent = adapter.info?.description || "Неизвестно";
            });
        } else {
            document.getElementById('gpu').textContent = "Недоступно";
        }
        ...
        if ('getBattery' in navigator) {
            navigator.getBattery().then(battery => {
                ...
                document.getElementById('batteryCharging').textContent = battery.charging ? "Заряжается" : "Не заряжается";
            });
        } else {
            document.getElementById('batteryLevel').textContent = "API не поддерживается";
        }
        ...
        document.getElementById('installedFonts').textContent = availableFonts.join(", ") || "Неизвестно";
        ...
        document.getElementById('fingerprint').textContent = fingerprintCanvas.toDataURL().slice(-32);
    </script>
</body>
</html>
"""
